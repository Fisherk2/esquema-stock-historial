...
| Spec-70 | Dockerfile & Docker Compose Prod | Non-root user `app`, OCI labels, `.dockerignore`, `docker-compose.prod.yml` self-contained, `.env.example` F0-F7, Makefile +demo/+docker-prod-up/+docker-prod-down | Alta | F7 | `Dockerfile`, `.dockerignore`, `docker-compose.prod.yml`, `.env.example`, `Makefile` | Spec-42, Spec-50, Spec-62 | [0/17] | Aprobado ✅ |
| Spec-71 | README Técnico & Demo Script & Documentación | README v1.0.0 con badges, ARCHITECTURE.md con diagramas Mermaid, API_REFERENCE.md 11 endpoints con errores, SETUP.md completo, `scripts/demo.sh`, CONTRIBUTING.md | Media | F7 | `README.md`, `docs/ARCHITECTURE.md`, `docs/API_REFERENCE.md`, `docs/SETUP.md`, `scripts/demo.sh`, `CONTRIBUTING.md` | Spec-62, Spec-70 | [0/12] | Aprobado ✅ |
| Spec-72 | CI/CD Pipeline | 5 gates secuenciales (lint→typecheck→test→coverage→docker-build), coverage sub-gates (≥80%/≥90%/≥85%), Docker build con `type=gha` cache, 0 deploy | Alta | F7 | `.github/workflows/ci.yml` | Spec-03, Spec-60, Spec-70 | [0/14] | Aprobado ✅ |
