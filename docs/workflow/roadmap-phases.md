...
| **F7: Despliegue & Documentación** | 2-3 días | ✅ Completada | Docker multi-stage (non-root, OCI labels), docker-compose prod, .env.example 17 vars, demo.sh, README v1.0.0, ARCHITECTURE.md, API_REFERENCE.md, SETUP.md, CONTRIBUTING.md, CI/CD 5 gates. **Hardening post-launch v1.0.2:** 2 commits /ship (bugs criticos: statement_timeout pool-wide, race condition SELECT FOR UPDATE, dead code, naive datetime, SET LOCAL scheduler; mejoras: SecurityHeadersMiddleware, pagination DB, Clean Architecture DTOs; 13 nuevos tests). 477 tests pass. |
